...
export function LeadForm({ variant = 'dark', defaultProduct = '', compact = false }: LeadFormProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!data.name.trim()) e.name = 'РЈРєР°Р¶РёС‚Рµ РёРјСЏ'
    if (!data.phone.trim()) e.phone = 'РЈРєР°Р¶РёС‚Рµ С‚РµР»РµС„РѕРЅ'
    else if (data.phone.replace(/\D/g, '').length < 10) e.phone = 'РќРµРєРѕСЂСЂРµРєС‚РЅС‹Р№ С‚РµР»РµС„РѕРЅ'
    if (!data.product.trim()) e.product = 'РЈРєР°Р¶РёС‚Рµ, С‡С‚Рѕ РЅСѓР¶РЅРѕ РёР·РіРѕС‚РѕРІРёС‚СЊ'
    if (data.file && data.file.size > MAX_FILE_SIZE) e.file = 'Р¤Р°Р№Р» Р±РѕР»СЊС€Рµ 10 РњР‘'
    if (data.file && !ALLOWED_TYPES.includes(data.file.type)) e.file = 'Р”РѕРїСѓСЃС‚РёРјС‹ JPG, PNG, PDF'
    setErrors(e)
  ...
  if (state === 'success') {
    return (
      <div className={`lead-success ${variant}`}>
        ...
        <h3 className="h3">Р—РђРЇР’РљРђ РћРўРџР РђР’Р›Р•РќРђ</h3>
        <p className="body">РЎРїР°СЃРёР±Рѕ! РњС‹ РїРѕР»СѓС‡РёР»Рё РІР°С€ Р·Р°РїСЂРѕСЃ Рё СЃРІСЏР¶РµРјСЃСЏ СЃ РІР°РјРё.</p>
        <button onClick={reset} className={`btn ${variant === 'dark' ? 'btn-outline-dark' : 'btn-outline'}`}>
          РћС‚РїСЂР°РІРёС‚СЊ РµС‰С‘ РѕРґРЅСѓ
        </button>
        <style>{`
          .lead-success {
            display: flex;
    ...
    <form onSubmit={handleSubmit} className={`lead-form ${variant} ${compact ? 'compact' : ''}`} noValidate>
      <div className="lead-form-row">
        <div className="lead-field">
          <label htmlFor="lf-name">РРјСЏ *</label>
          <input
            id="lf-name"
            type="text"
            value={data.name}
        ...
        <div className="lead-field">
          <label htmlFor="lf-phone">РўРµР»РµС„РѕРЅ *</label>
          <input
            id="lf-phone"
            type="tel"
            value={data.phone}
      ...
      <div className="lead-field">
        <label htmlFor="lf-product">Р§С‚Рѕ РЅСѓР¶РЅРѕ РёР·РіРѕС‚РѕРІРёС‚СЊ? *</label>
        <input
          id="lf-product"
          type="text"
          value={data.product}
      ...
      <div className="lead-field">
        <label htmlFor="lf-comment">РљРѕРјРјРµРЅС‚Р°СЂРёР№</label>
        <textarea
          id="lf-comment"
          value={data.comment}
      ...
      <div className="lead-field">
        <label>РџСЂРёРєСЂРµРїРёС‚СЊ С„РѕС‚Рѕ РёР»Рё С‡РµСЂС‚С‘Р¶</label>
        <div
        ...
        >
          ...
          {data.file ? (
            ...
            <div className="lead-upload-placeholder">
              <Upload size={20} />
              <span>РџРµСЂРµС‚Р°С‰РёС‚Рµ С„Р°Р№Р» РёР»Рё РЅР°Р¶РјРёС‚Рµ</span>
              <span className="lead-upload-hint">JPG, PNG, PDF вЂ” РґРѕ 10 РњР‘</span>
            </div>
          )}
        </div>
      ...
      {state === 'error' && (
        <div className="lead-form-error">
          РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РїСЂР°РІРёС‚СЊ Р·Р°СЏРІРєСѓ. РџРѕР·РІРѕРЅРёС‚Рµ РЅР°Рј РёР»Рё РїРѕРїСЂРѕР±СѓР№С‚Рµ РµС‰С‘ СЂР°Р·.
        </div>
      )}
      <button type="submit" className={`btn btn-primary lead-form-submit`} disabled={state === 'loading'}>
        {state === 'loading' ? 'РћРўРџР РђР’РљРђ...' : 'РџРћР›РЈР§РРўР¬ Р РђРЎР§РЃРў'}
        {state !== 'loading' && <ArrowUpRight size={18} />}
      </button>
      <p className={`lead-form-privacy ${isDark ? 'dark' : ''}`}>
        РќР°Р¶РёРјР°СЏ РєРЅРѕРїРєСѓ, РІС‹ СЃРѕРіР»Р°С€Р°РµС‚РµСЃСЊ СЃ{' '}
        <a href="/privacy">РїРѕР»РёС‚РёРєРѕР№ РєРѕРЅС„РёРґРµРЅС†РёР°Р»СЊРЅРѕСЃС‚Рё</a>
      </p>
      <style>{`
        .lead-form {
          display: flex;
